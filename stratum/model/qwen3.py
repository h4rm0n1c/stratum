"""Qwen3 model architecture adapter for Stratum.

Includes capability-dispatched flash attention for mixed RTX 3080/V100 runs.

Qwen3 is the non-MoE variant. It uses per-head q_norm/k_norm (like Qwen3.5)
but does NOT have the gate projection in the attention output. All public Qwen3
models are full-attention only; the sliding-window path is defensive-only.
"""
...
class Qwen3FlashAttention(Qwen3Attention):
    """Qwen3 attention using capability-dispatched flash-attention.

    Dispatches to the right backend based on GPU architecture:
    - sm_70 (V100): flash_attn_v100
    - sm_80+ (Ampere+): standard flash_attn

    Qwen3Attention has q_norm/k_norm but no gate projection (unlike Qwen3.5).
    ...
    def forward(
        ...
        **kwargs: Any,
        ...
        if kwargs.get("output_attentions", False):
            raise ValueError("Qwen3FlashAttention does not return attention weights")
        ...
        # Qwen3: q_norm/k_norm applied after projection, no gate.
        ...
        if flash_backend is not None:
            ...
            try:
                ...
                raise RuntimeError(
                    f"{flash_backend.name} failed for Qwen3 layer {self.layer_idx}; "
        ...
        if flash_backend is None:
            if hidden_states.device.type == "cuda":
                raise RuntimeError(
                    f"no flash-attention backend available for Qwen3 layer {self.layer_idx} "
...
class Qwen3ForCausalLMPrefix(nn.Module):
    ...
    Builds a causal_mask dict with ``"full_attention"`` and optionally
    ``"sliding_attention"`` keys.  All public Qwen3 models are full-attention
    only; the sliding-window key is populated defensively when
    ``has_sliding_layers`` is True.  In flash mode both masks are None.
    """
    ...
    def __init__(
        ...
        *,
        ...
        output_router_logits: bool = False,  # unused for Qwen3, accepted for compat
    ):
        super().__init__()
        ...
        self.has_sliding_layers = "sliding_attention" in getattr(
...
class Qwen3ForCausalLMWrappedLayer(nn.Module):
    """One Qwen3 decoder layer wrapper with optional activation checkpointing."""
    ...
    def forward(self, input_data):
        ...
        # Route attention mask from dict using this layer's attention type.
        ...
        if attention_type in causal_mask:
            ...
            attn_mask = causal_mask.get("full_attention")
        else:
            attn_mask = None
...
class Qwen3ForCausalLMPostfix(nn.Module):
    ...
    Qwen3 (non-MoE) has no router auxiliary loss.
    router_aux_loss_coef is accepted for interface compatibility but ignored.
    """
...
class Qwen3Arch(ModelArch):
    ...
    def build_wrapped_layer(self, layer, idx, **kwargs):
        return Qwen3ForCausalLMWrappedLayer(
            layer,
            idx=idx,
    ...
    def build_postfix(self, model, **kwargs):
        return Qwen3ForCausalLMPostfix(
            model,
    ...
    def build(self, hf_model, tensor_split=None, device_ids=None, **kwargs):
        ...
        _patch_qwen3_attention(core, layer_indices=flash_layer_indices)
...
def _patch_qwen3_attention(
    model,
    layer_indices: Optional[set[int]] = None,
):
    """Replace Qwen3Attention with capability-dispatched flash attention.
    ...
    for idx, layer in enumerate(core.model.layers):
        ...
        if not isinstance(old_attn, Qwen3Attention):
            continue
        if isinstance(old_attn, Qwen3FlashAttention):
            if layer_indices is None or idx in layer_indices:
                patched += 1
            continue

        new_attn = Qwen3FlashAttention(old_attn.config, layer_idx=idx)
    ...
    vwrite(f"Patched {patched} Qwen3 attention layers with capability-dispatched flash attention")
